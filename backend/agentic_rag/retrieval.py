...
def retrieve_amp_info(query: str) -> List[Dict[str, Any]]:
    ...
    if not weaviate_client or not embedding_model:
        return [{"error": "Database client or embedding model not initialized. Check server logs."}]
    ...
    try:
        ...
        if not results:
            return [{"message": "No relevant information found in the database for your query."}]
