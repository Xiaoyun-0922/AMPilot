...
import pandas as pd
import weaviate
from configuration import DATA_DIR, WEAVIATE_COLLECTION_NAME, WEAVIATE_URL
...
def main():
    ...
    if client.collections.exists(WEAVIATE_COLLECTION_NAME):
        print(
            f"Collection '{WEAVIATE_COLLECTION_NAME}' already exists. Deleting it for fresh import."
        )
    ...
    print(
        f"Generating embeddings for {len(df)} records (this may take a few minutes on GPU)..."
    )
    ...
    print(
        f"Ingestion complete! {len(records)} records were successfully imported with the new embedding strategy."
    )
    client.close()
